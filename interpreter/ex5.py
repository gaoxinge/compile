...
class Interpreter(object):
        ...
        def patt(self):
                if self.lexer.text.find('(') == -1: return self.expr()
                else:
                        index = 0
                        for i in range(self.lexer.text.find('(') + 1, len(self.lexer.text)):
                                if self.lexer.text[i] == '(': index += 1
                                if self.lexer.text[i] == ')': index -= 1
                                if index == -1:
                                        result = Interpreter(Lexer(self.lexer.text[self.lexer.text.find('(') + 1:i])).patt()
                                        result = Interpreter(Lexer(self.lexer.text[0:self.lexer.text.find('(')] + repr(result) + self.lexer.text[i + 1:len(self.lexer.text)])).patt()
                                        return result
                self.error()
